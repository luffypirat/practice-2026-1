...
class TodoStates(StatesGroup):
    ...
    delete_cmd_keyboard_state = State()
...
async def list_cmd(message: Message, state: FSMContext):
    ...
    for i, done_row in enumerate(rows, start=1):
        num_id_dic[i] = done_row[0]
        icon = "✅" if done_row[2] == 1 else "❌"
        result += str(i) + ". " + done_row[1] + icon + "\n"  # выводим порядковый номер"
    if not rows:
        await message.answer("У вас нет задач!")
        return
    await message.answer(result, reply_markup=tasks_keyboard(rows))
    await state.update_data(num_id_dic=num_id_dic)
...
async def delete_keyboard_button(message: Message, state: FSMContext):
    await message.answer("Напишите номер задачи, которую нужно удалить:")
    await state.set_state(TodoStates.delete_cmd_keyboard_state)


@router.message(TodoStates.delete_cmd_keyboard_state)
async def delete_cmd_keyboard(message: Message, state: FSMContext):
    dic = await state.get_data()
    num_id_dic = dic.get("num_id_dic", {})
    try:
        task_id = int(message.text)
        real_id = num_id_dic[task_id]
    except (IndexError, ValueError, KeyError):
        await message.answer("Пожалуйста, укажите корректный номер задачи!")
        return
    check = await delete_task(message.from_user.id, real_id)
    if check == 0:
        await message.answer("Задачи под данным номером не существует!")
    else:
...
@router.message(F.text.in_({"О боте"}))
async def about_cmd(message: Message, state: FSMContext):
    await message.answer("""
Проектная практика 2025, Московский политех
Бот для списка дел. V0.1

Работу выполнили студенты группы 251-336:
- Останин Платон Валерьевич;
- Чулков Егор Дмитриевич;
- Нагарёв Иван Олегович.
